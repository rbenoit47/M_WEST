...
"""
"""
    Python script to run the MC2 model.
...
#os.putenv('AFSISIO', os.path.join(BIN_DIR, 'dfiles'))
...
os.putenv('rep_from_which_model_is_launched', WORKING_DIR)
...
################################################################################
# Main body of RunMC2dm script
# Run command. The leading and trailing " are to allow pathnames with spaces.
COMMAND = '"' + os.path.join(BIN_DIR, EXECUTABLE) + '"'
print 'Running MC2 Model - ', EXECUTABLE

# Run with or without a log file
if LOG_FILENAME == '':
    print '  Job Start: ', time.strftime("%Y/%m/%d %H:%M:%S", time.localtime())
    os.system(COMMAND)
    print '  Job Finish:', time.strftime("%Y/%m/%d %H:%M:%S", time.localtime())
else:
    now = time.localtime()
