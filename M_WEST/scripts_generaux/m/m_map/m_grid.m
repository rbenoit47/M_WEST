...
if ~isempty(xtick),
 ...
 ik=1:size(X,2);
%RB rotang must be double with MATLAB 2015
 for k=ik,
   [rotang(k), horizk, vertk] = upright(rotang(k), horiz, vert);
	%  text needs double type coordinates  RB august 2015
    if verLessThan('matlab','8.5')
        rotk=rotang(k);
    else
        rotk=double(rotang(k));
    end
    % 		'rotation',double(rotang(k)),...
    text(double(xx(k)),double(yy(k)),labs{k},'horizontalalignment',horizk,'verticalalignment',vertk, ...
		'rotation',rotk,...
		'fontsize',gfontsize*scl(k),'color',gcolor,...
...
if ~isempty(ytick),
 ...
 % Finally - the labels!
 ik=1:size(X,2);
%RB  rotang double for this MATLAB version 2015
 for k=ik,    
     if verLessThan('matlab','8.5')
         rotk=rotang(k);
     else
   ...
   [rotang(k), horizk, vertk] = upright(rotang(k), horiz, vert);
   text(double(xx(k)),double(yy(k)),labs{k},'horizontalalignment',horizk,'verticalalignment',vertk,...
        'rotation',rotk,'fontsize',gfontsize*scl(k),'color',gcolor,...
